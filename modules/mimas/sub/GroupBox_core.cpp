...
static int QWidget_setAttribute(lua_State *L) {
  try {
    GroupBox *self = *((GroupBox**)dubL_checksdata(L, 1, "mimas.GroupBox"));
    if (!self) throw dub::Exception("Using deleted mimas.GroupBox in setAttribute");
...
static int QWidget_setFocus(lua_State *L) {
  try {
    GroupBox *self = *((GroupBox**)dubL_checksdata(L, 1, "mimas.GroupBox"));
    if (!self) throw dub::Exception("Using deleted mimas.GroupBox in setFocus");
...
static int QWidget_setFocusPolicy(lua_State *L) {
  try {
    GroupBox *self = *((GroupBox**)dubL_checksdata(L, 1, "mimas.GroupBox"));
    if (!self) throw dub::Exception("Using deleted mimas.GroupBox in setFocusPolicy");
