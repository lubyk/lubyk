...
static int QWidget_adjustSize(lua_State *L) {
  try {
    Menu *self = *((Menu**)dubL_checksdata(L, 1, "mimas.Menu"));
    if (!self) throw dub::Exception("Using deleted mimas.Menu in adjustSize");
    self->adjustSize();
    return 0;
  } catch (std::exception &e) {
...
static int QWidget_setAttribute(lua_State *L) {
  try {
    Menu *self = *((Menu**)dubL_checksdata(L, 1, "mimas.Menu"));
    if (!self) throw dub::Exception("Using deleted mimas.Menu in setAttribute");
...
static int QWidget_setFocus(lua_State *L) {
  try {
    Menu *self = *((Menu**)dubL_checksdata(L, 1, "mimas.Menu"));
    if (!self) throw dub::Exception("Using deleted mimas.Menu in setFocus");
...
static int QWidget_setFocusPolicy(lua_State *L) {
  try {
    Menu *self = *((Menu**)dubL_checksdata(L, 1, "mimas.Menu"));
    if (!self) throw dub::Exception("Using deleted mimas.Menu in setFocusPolicy");
