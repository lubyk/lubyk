...
/** zmq::Socket::Socket(int type, zmq::Context *ctx)
 * include/zmq/Socket.h:75
 */
static int Socket_Socket(lua_State *L) {
  try {
    int type = dub_checkint(L, 1);
    Context *ctx = *((Context **)dub_checksdata(L, 2, "zmq.Context"));
    Socket *retval__ = new Socket(type, ctx);
