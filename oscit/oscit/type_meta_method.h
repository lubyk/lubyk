  ...
  virtual const Value trigger (const Value &url) {
    ...
    if (target != NULL) {
      ...
      if (!type.is_nil()) {
        ...
        if (!type.is_list()) return ErrorValue(INTERNAL_SERVER_ERROR, "Invalid meta type. Should be a list (found '").append(type.type_tag()).append("').");
        // get current value
    ...
    return ErrorValue(NOT_FOUND_ERROR, url.c_str());
  }
};
