...
void Root::send_reply(const IpEndpointName *remote_endpoint, const std::string &url, const Value &val) {
  
  if (val.is_error()) {
    if (remote_endpoint) OscSend(*remote_endpoint).send("/.error", val);
  } else {
    ...
    if (remote_endpoint) {
      ...
      OscSend(*remote_endpoint).send("/.reply", res);
    }
