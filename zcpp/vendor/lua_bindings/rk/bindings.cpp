...
static int rk_drawMatrix(lua_State *L) {
  try {
    ...
    std::string *s = new std::string("rk.drawMatrix: ");
    s->append(e.what());
    ...
    lua_pushstring(L, "rk.drawMatrix: Unknown exception");
    lua_error(L);
    return 0;
  }
}
...
static int rk_drawTexture(lua_State *L) {
  try {
    ...
    if (top__ < 6) {
      ...
      bool flip_y = lua_toboolean(L, 6);
      drawTexture(tex, start_x, start_y, end_x, end_y, flip_y);
    }
    return 0;
  } catch (std::exception &e) {
    std::string *s = new std::string("rk.drawTexture: ");
    s->append(e.what());
    lua_pushstring(L, s->c_str());
    delete s;
    lua_error(L);
    ...
    lua_pushstring(L, "rk.drawTexture: Unknown exception");
    lua_error(L);
    return 0;
  }
}
...
static int rk_makeTexture(lua_State *L) {
  try {
    ...
    std::string *s = new std::string("rk.makeTexture: ");
    s->append(e.what());
    lua_pushstring(L, s->c_str());
    delete s;
    lua_error(L);
    ...
    lua_pushstring(L, "rk.makeTexture: Unknown exception");
    lua_error(L);
    return 0;
  }
}
